...
public class FlexAssembler extends PayloadBlock{
    ...
    public void init(){
        ...
        consPayload = new ConsumePayloadDynamic((FlexAssemblerBuild build) -> {
            AssemblerUnitPlan plan = build.plan();
            return plan == null || plan.requirements == null ? new Seq<PayloadStack>() : plan.requirements;
        });
        consItem = new ConsumeItemDynamic((FlexAssemblerBuild build) -> {
            AssemblerUnitPlan plan = build.plan();
            return plan == null || plan.itemReq == null ? ItemStack.empty : plan.itemReq;
        });
        consume(consPayload);
        consume(consItem);
        consume(new ConsumeLiquidsDynamic((FlexAssemblerBuild build) -> {
            AssemblerUnitPlan plan = build.plan();
    ...
    public void afterPatch(){
        ...
        if(capacities == null || capacities.length != Vars.content.items().size){
            capacities = new int[Vars.content.items().size];
        }
        for(AssemblerUnitPlan plan : plans){
            if(plan.itemReq != null){
    ...
    public void setStats(){
        ...
        stats.add(Stat.output, table -> {
            ...
            for(AssemblerUnitPlan plan : plans){
                ...
                byTier.computeIfAbsent(required, k -> new Seq<>()).add(plan);
            }
                ...
                for(AssemblerUnitPlan plan : group){
                    table.table(Tex.pane, t -> {
                        ...
                        if(plan.unit.unlockedNow()){
                            t.image(plan.unit.uiIcon).scaling(Scaling.fit).size(40).pad(10).left();
                            t.table(info -> {
                                info.left();
                                ...
                                if(required > 0){
                                    info.row();
                                    info.add(Core.bundle.format("flexassembler.tier.stat", required))
    ...
    public class FlexAssemblerBuild extends PayloadBlock.PayloadBlockBuild<Payload>{
        ...
        public void updateModules(FlexAssemblerModule.FlexAssemblerModuleBuild build){
        ...
        public boolean shouldConsume(){
            ...
            return enabled &&
                    !wasOccupied &&
                    Units.canCreate(team, current.unit) &&
        ...
        public void drawSelect(){
            ...
            Drawf.dashRect(
                    Tmp.c1.set(Pal.accent).lerp(Pal.remove, invalidWarmup),
                    getRect(Tmp.r1)
            );
        }
        @Override
        public void display(Table table){
            ...
            table.table(t -> {
                ...
                for(FlexAssemblerModule.FlexAssemblerModuleBuild module : modules){
                    ...
                    t.image(module.block.uiIcon).size(iconMed).padRight(4);
                    prev = module.block;
                }
                AssemblerUnitPlan current = plan();
                if(current != null){
                    t.label(() -> "[accent] -> []" + current.unit.emoji() + " " + current.unit.localizedName);
                }
        ...
        public void buildConfiguration(Table table){
            ...
            for(int i = 0; i < available.size; i++){
                ...
                AssemblerUnitPlan candidate = available.get(i);
                boolean chosen = locked && current == candidate;
                int index = plans.indexOf(candidate);
                Button button = new Button(Tex.button);
                button.table(inner -> {
                    inner.image(candidate.unit.uiIcon).size(30f).padBottom(4f);
                    inner.row();
                    inner.add(candidate.unit.localizedName)
                            .color(chosen ? Pal.accent : Color.lightGray);
                }).pad(8);
                button.clicked(() -> configure(index));
                grid.add(button).size(80f, 80f).pad(4f);
            }
            table.add(new ScrollPane(grid)).grow().maxHeight(400f).row();
            if(locked){
                table.button(
                        Core.bundle.get("flexassembler.deselect"),
                        () -> configure(NO_PLAN)
        ...
        public void updateTile(){
            ...
            droneWarmup = Mathf.lerpDelta(
                    droneWarmup,
                    units.size < dronesCreated ? powerStatus : 0f,
                    0.1f
            );
            ...
            Vec2 spawn = getUnitSpawn();
            if(payload != null && moveInPayload() && !wasOccupied){
                yeetPayload(payload);
                payload = null;
            }
            for(int i = 0; i < units.size; i++){
                ...
                ai.targetPos.trns(
                        i * 90f + 45f,
                        myAreaSize / 2f * Mathf.sqrt2 * tilesize
                ).add(spawn);
        ...
        private boolean planTimeValid(AssemblerUnitPlan plan){
            return plan.time > 0f;
        }
        ...
        public void spawned(){
            ...
            Vec2 spawn = getUnitSpawn();
            consume();
            Unit unit = current.unit.create(team);
        ...
        public void draw(){
            ...
            if(sideRegion1 != null && sideRegion2 != null){
                Draw.rect(
                        rotation >= 2 ? sideRegion2 : sideRegion1,
                        x,
                        y,
                        rotdeg()
                );
            ...
            Draw.draw(Layer.blockBuilding, () -> {
                ...
                Shaders.blockbuild.region = current.unit.fullIcon;
            ...
            for(Unit drone : units){
                if(!(drone.controller() instanceof AssemblerAI ai) || !ai.inPosition()){
                    continue;
                }
                ...
                Drawf.buildBeam(
                        ...
                        spawn.x,
                        spawn.y,
                        current.unit.hitSize / 2f
                );
            }
            Fill.square(spawn.x, spawn.y, current.unit.hitSize / 2f);
            Draw.reset();
            Draw.z(Layer.buildBeam);
            ...
            Draw.reset();
            float outSize = current.unit.hitSize + 9f;
            if(invalidWarmup > 0){
                Lines.stroke(
                        2f,
        ...
        public boolean checkSolid(Vec2 v, boolean same){
            ...
            UnitType output = current.unit;
            float hsize = output.hitSize * 1.4f;
            return (
                    !output.flying &&
                            collisions.overlapsTile(
                                    Tmp.r1.setCentered(v.x, v.y, output.hitSize),
        ...
        public void yeetPayload(Payload payload){
            ...
            Fx.payloadDeposit.at(
                    ...
                    payload.y(),
                    rot,
                    new YeetData(spawn.cpy(), payload.content())
            );
        ...
        public boolean acceptPayload(Building source, Payload payload){
            AssemblerUnitPlan current = plan();
            if(current == null || current.requirements == null){
                return false;
            }
            return (
                    this.payload == null ||
                            ...
                            stack ->
                                    stack.item == payload.content() &&
                                            blocks.get(payload.content()) <
                                                    Mathf.round(
                                                            stack.amount * state.rules.unitCost(team)
                                                    ) -
                                                            (
                                                                    source instanceof FlexAssemblerModule.FlexAssemblerModuleBuild &&
                                                                            this.payload != null &&
        ...
        public int getMaximumAccepted(Item item){
            if(item == null || capacities == null || item.id < 0 || item.id >= capacities.length){
                return 0;
            }
        ...
        public boolean acceptItem(Building source, Item item){
            ...
            return current != null &&
                    ...
                    Structs.contains(current.itemReq, stack -> stack.item == item);
        }
        @Override
    ...
    public static class FlexAssemblerModule extends PayloadBlock{
        ...
        public FlexAssemblerBuild getLink(Team team, int x, int y, int rotation){
            Building found = indexer.getFlagged(team, BlockFlag.unitAssembler).find(
                    b ->
            ...
            return found instanceof FlexAssemblerBuild build ? build : null;
        }
        public Building createBuilding(){
